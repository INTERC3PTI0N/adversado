...
export const LaserFlow = ({
  // Defaulted rather than left bare: without these, TypeScript infers them as
  // required props from the destructuring and every call site fails the build
  // unless it passes all three. `dpr: undefined` is the component's own
  // "derive from the device" path, so the default is also the correct value.
  className = '',
  style = undefined,
  wispDensity = 1,
  dpr = undefined,
  ...
  useEffect(() => {
    ...
    // Local edit to the vendored component: upstream loops unconditionally,
    // which made this the only shader on the site that ignored
    // `prefers-reduced-motion` — Silk, the cinematic field and the cursor
    // trail all honour it. Reduced motion still gets the beam, fully faded in
    // and lit, just not animating: a composed static state rather than an
    // absence, which is what the project's accessibility gates ask for.
