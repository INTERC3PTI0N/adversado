...
//
// Shader source (vertSrc/fragSrc) and all uniform/render logic are verbatim
// from the supplied standalone HTML. The only adaptation is the React shell
// around it, and the interaction: tilt/rotate follow the pointer on hover
...
export function EventHorizon({
  ...
  useEffect(() => {
    ...
    const rotationSpeedVal = rotationSpeed;
    const diskIntensityVal = diskIntensity;
    const starsOnlyVal = 0.0;
    let tiltVal = 0.0;
    let rotateVal = 0.0;
    const bhCenterX = 0.0;
    const bhCenterY = 0.0;
    const bhScaleVal = 0.0;
    const chromaticVal = chromatic;

    // Hover, not drag: pointer position across the viewport maps straight
    // onto the same tiltVal/rotateVal the original drove from mouse deltas,
    // eased so a fast move glides instead of snapping.
    let aimTilt = 0;
    let aimRotate = 0;
    const onMove = (e: PointerEvent) => {
      ...
      aimRotate = nx * 1.2;
      aimTilt = Math.max(-0.8, Math.min(0.8, ny * 0.9));
    };
    ...
    function render(now: number) {
      ...
      tiltVal += (aimTilt - tiltVal) * 0.05;
      rotateVal += (aimRotate - rotateVal) * 0.05;
  ...
  }, [rotationSpeed, diskIntensity, chromatic]);
